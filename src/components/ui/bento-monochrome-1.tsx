...
const flows = [
  {
    id: "01",
    variant: "orbit",
    meta: "Status do Ativo",
    title: "ATIVO DIGITAL: Master Data",
    description:
      "Infraestrutura sênior focada em autoridade. Projetada para dominar o mercado, atrair grandes frotistas e converter leads qualificados.",
    statLabel: "PRONTO PARA ESCALAR",
    statValue: "48 hrs",
  },
  {
    id: "02",
    ...
    description:
      "Design imersivo que coloca sua marca anos-luz à frente da concorrência tradicional.",
    statLabel: "MÉTRICA",
    statValue: "+ 100%",
  },
  {
    ...
    variant: "wave",
    meta: "HIGH-TECH",
    title: "Interação & Conversão",
    description:
      "Interface projetada para facilitar o contato de novos postos e fechar contratos de alto valor. ",
    statLabel: "HIGH-TECH",
    statValue: "UX",
  },
  {
    ...
    variant: "spark",
    meta: "Performance",
    title: "Potencial De Tráfego & ADS",
    description:
      "Estrutura técnica otimizada para campanhas de Google e Meta Ads com o menor custo por lead do setor.",
    statLabel: "Performance Mensal",
    statValue: "ROI",
  },
  {
    id: "05",
    ...
    description:
      "Integração inteligente com Google Maps e SEO Local para garantir que a Master Data seja a autoridade nº1 em Uberlândia e região.",
    statLabel: "DOMÍNIO GEOGRÁFICO:",
    statValue: "TRIÂNGULO MINEIRO",
  },
];
...
const palettes = {
  ...
  light: {
    ...
    card: "bg-white/5",
    cardBorder: "border-white/10",
    metric: "bg-white/5 border-white/10 text-blue-100/70",
    ...
    focusGlow: "rgba(147, 197, 253, 0.12)",
    iconStroke: "#93C5FD",
    iconTrail: "rgba(147, 197, 253, 0.35)",
  },
};
...
function Bento3Section() {
  const [theme, setTheme] = useState<"light" | "dark">(() => getRootTheme() as "light" | "dark");
  ...
  useEffect(() => {
    ...
    const syncTheme = () => {
      const next = getRootTheme() as "light" | "dark";
  ...
  return (
    <div
      ...
      style={{ ...containerStyle, background: 'linear-gradient(180deg, #060E1C 0%, #081629 50%, #060E1C 100%)' }}
    >
      <div
      ...
      <section
        ref={sectionRef}
        className={`bento3-section relative z-10 mx-auto flex max-w-6xl flex-col gap-12 py-24 md:gap-16 ${introReady && visible ? "" : "opacity-0"
          }`}
      >
        <div
          className={`bento3-hero-pill mx-auto flex w-full max-w-xl items-center justify-between gap-4 rounded-full border px-5 py-3 text-[11px] uppercase tracking-[0.5em] transition-all duration-700 ${introReady ? "opacity-100 translate-y-0" : "translate-y-5 opacity-0"
            } ${palette.capsule}`}
        >
          ...
          <span className="font-medium">MASTER DATA CORE V1.0</span>
        </div>
        ...
        <header className="flex flex-col gap-10 md:flex-row md:items-end md:justify-between">
          <div className="space-y-5">
            <div className="inline-flex items-center gap-3 rounded-full px-3 py-1 text-xs uppercase tracking-[0.4em] text-current">
              ...
              STATUS: PRONTO PARA ESCALAR E VENDER
            </div>
            <h1 className={`text-3xl font-normal tracking-tight leading-tight sm:text-4xl md:text-5xl ${palette.heading}`}>
              Sua Marca em um Novo Patamar de Mercado
            </h1>
            <p className={`max-w-2xl text-sm sm:text-base md:text-lg ${palette.muted}`}>
              Muito além de uma página, desenvolvemos uma infraestrutura digital projetada para expansão, autoridade e conversão imediata de novos clientes.
            </p>
          </div>

          <div className="flex flex-col items-start gap-4 md:items-end">
            <div className="flex flex-wrap items-center gap-3">
              <span className={`text-xs uppercase tracking-[0.35em] ${palette.muted}`}>Site Alto Padrão</span>
              <button
                type="button"
                onClick={toggleTheme}
              ...
              >
                <span className="relative flex h-6 w-6 items-center justify-center">
                  <span
                    className={`pointer-events-none absolute inset-0 rounded-full border opacity-40 ${theme === "dark" ? "border-white/40" : "border-neutral-400/60"
                      }`}
                  />
                  <span
                    className={`h-3 w-3 rounded-full transition-all duration-500 ${theme === "dark" ? "bg-white" : "bg-neutral-900"
                      }`}
                  />
                </span>
            ...
            <p className={`max-w-xs text-xs ${palette.muted}`}>
              O Novo Ativo Digital da Master Data
            </p>
          </div>
        </header>
        ...
        <footer className="bento3-footer flex flex-col gap-5 border-t border-dashed border-current/20 pt-8 text-sm md:flex-row md:items-center md:justify-between">
          <div className={`flex flex-col gap-2 ${palette.muted}`}>
            <span className="text-xs uppercase tracking-[0.35em]">Sua infraestrutura digital não é apenas um site</span>
            <span className="text-base font-medium text-current">É o centro de comando para a liderança do mercado de automação em Minas Gerais.</span>
          </div>
    ...
    >
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start">
        ...
        <div className="flex flex-col gap-4 lg:flex-1">
          ...
          <h3 className={`text-xl font-normal tracking-tight leading-tight sm:text-2xl ${palette.heading}`}>{flow.title}</h3>
